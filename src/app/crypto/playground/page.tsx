...
import { ArrowLeft, KeyRound, ArrowRightLeft, Cpu } from "lucide-react";

// Import komponen kanvas yang sudah kita buat
...
type CryptoSimType = 'caesar' | 'vigenere' | 'affine';
  ...
  return (
    <main className="h-screen bg-[#0b1120] text-slate-100 flex flex-col overflow-hidden">
      ...
      <header className="shrink-0 p-3 md:p-5 flex flex-col sm:flex-row sm:items-center justify-between border-b border-slate-800/60 bg-slate-900/80 backdrop-blur-md z-20 gap-4">
        ...
        {/* Tengah/Kanan: Menu Tab Algoritma */}
        <div className="flex justify-start sm:justify-end gap-2 overflow-x-auto custom-scrollbar pb-1 sm:pb-0 w-full sm:w-3/4">
           <button 
             onClick={() => setActiveSim('caesar')} 
             className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs md:text-sm font-bold whitespace-nowrap transition-all border ${activeSim === 'caesar' ? 'bg-indigo-600 text-white border-indigo-500 shadow-lg shadow-indigo-500/20' : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700'}`}
           >
             <ArrowRightLeft size={16}/> Caesar Cipher
           </button>
           
           <button 
             onClick={() => setActiveSim('affine')} 
             className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs md:text-sm font-bold whitespace-nowrap transition-all border ${activeSim === 'affine' ? 'bg-indigo-600 text-white border-indigo-500 shadow-lg shadow-indigo-500/20' : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700'}`}
           >
             <Cpu size={16}/> Affine Cipher
           </button>

           <button 
             onClick={() => setActiveSim('vigenere')} 
             className={`flex items-center gap-2 px-4 py-2 rounded-xl text-xs md:text-sm font-bold whitespace-nowrap transition-all border ${activeSim === 'vigenere' ? 'bg-indigo-600 text-white border-indigo-500 shadow-lg shadow-indigo-500/20' : 'bg-slate-800 text-slate-400 border-slate-700 hover:bg-slate-700'}`}
           >
