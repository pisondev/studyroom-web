...
export default function CryptoLearn() {
  ...
  const chapterIndex = cryptoCourseData.findIndex(c => c.id === chapterId);
  const currentChapter = cryptoCourseData[chapterIndex];
  ...
  useEffect(() => {
    ...
    const savedProgress = localStorage.getItem(`crypto_resume_${chapterId}`);
    if (savedProgress) {
  ...
  useEffect(() => {
    if (isMounted && currentChapter) {
      localStorage.setItem('crypto_last_chapter', chapterId); 
      localStorage.setItem(`crypto_resume_${chapterId}`, page.toString());
      
      const currentMax = parseInt(localStorage.getItem(`crypto_progress_${chapterId}`) || '0');
      if (page > currentMax) {
        localStorage.setItem(`crypto_progress_${chapterId}`, page.toString());
      }
    }
  }, [page, chapterId, isMounted, currentChapter]);

  const isLastSlide = page === slides.length - 1;
  const hasNextChapter = chapterIndex < cryptoCourseData.length - 1;
  ...
  const paginate = useCallback((newDirection: number) => {
    ...
    if (newPage < 0) { 
      if (chapterIndex > 0) router.push(`/crypto/learn/${cryptoCourseData[chapterIndex - 1].id}`); 
      return; 
    }
    if (newPage >= slides.length) {
      if (hasNextChapter) router.push(`/crypto/learn/${cryptoCourseData[chapterIndex + 1].id}`);
      else router.push('/crypto');
      return;
    }
        ...
        <AnimatePresence initial={false} custom={direction} mode="wait">
          ...
          >
            <div className={`bg-slate-800/40 rounded-3xl border border-slate-700/50 backdrop-blur-xl shadow-2xl flex flex-col ${isCanvasMode ? 'p-4 h-full' : 'p-6 md:p-12'}`}>
              ...
              {/* Canvas Renderer Khusus Kriptografi */}
